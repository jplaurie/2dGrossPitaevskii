...
#ifdef __CUDACC__
#define GP2D_HD __host__ __device__
#else
#define GP2D_HD
#endif
...
template <class T>
GP2D_HD T integrationStageA(Integrator method, double h, std::size_t i,
...
template <class T>
GP2D_HD T integrationStageB(Integrator method, std::size_t i,
...
template <class T>
GP2D_HD T integrationStageC(std::size_t i, CoefficientPointers<T> c, T w, T n1,
                            T n3) {
  return c.e1[i] * w + c.q4[i] * n1 + c.q5[i] * n3;
}

template <class T>
GP2D_HD T integrationFinish(Integrator method, double h, std::size_t i,
