#!/usr/bin/env python3
"""Create an MP4 or GIF of wave-action and energy spectra."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--frequency-density", action="store_true")
...
def main() -> None:
    ...
    def values(frame: int):
        ...
        x, x_label = spectral_abscissa(k, args.x_axis, coefficient, chemical_potential)
        wave = np.asarray(rows["wave_action_spectrum"])
        energy = np.asarray(rows["quadratic_energy_spectrum"])
        if args.x_axis == "frequency" and args.frequency_density:
            wave = density_in_frequency(k, wave, coefficient)
            energy = density_in_frequency(k, energy, coefficient)
        mask = np.isfinite(x) & np.isfinite(wave) & np.isfinite(energy)
        mask &= (x > 0.0) & (wave > 0.0) & (energy > 0.0)
    ...
    ylabels = [r"$n_k$", r"$E_k^{(2)}$"]
    if args.x_axis == "frequency" and args.frequency_density:
        ylabels = [r"$n_\omega$", r"$E_\omega^{(2)}$"]
    for axis, ylabel, limits in zip(
        axes,
        ylabels,
        [(wave_all.min(), wave_all.max()), (energy_all.min(), energy_all.max())],
    ):
        axis.set_xscale("log")
        axis.set_yscale("log")
