...
Programme to solve the 2D GP equation  iPSI_t = - 0.5 * Delta PSI  + 0.5*g* (|PSI|^2 - mu ) * PSI + forcing + dissipation
...
if(FLAG_FORCE_TYPE != 0){
	...
	if(FLAG_FORCE_EXP == true){
		...
		forcing(famp,kf,dk); //Initialise Forcing
	} 


	if(FLAG_FORCE_TYPE == 3){
		for(int j = 0; j < Ny; j++){
			for(int i = 0; i < Nx; i++){
          		if(famp(i,j) > 0){  
            		famp(i,j) = abs(psi_hat(i,j));          			
          		}
        	}
      	}
...
for( int time_step = 1; time_step <= nsteps; time_step++ ){
	...
	if(FLAG_TIMESTEP_ETDRK == true){
		...
		if(FLAG_FORCE_TYPE == 1){
			psi_hat += pow(dt,0.5)*noise; // euler noise
  		}
  		else if(FLAG_FORCE_TYPE == 2 || FLAG_FORCE_TYPE == 3){
  			for(int j = 0; j < Ny; j++){
				for(int i = 0; i < Nx; i++){
          			if(famp(i,j) > 0){  
            			psi_hat(i,j) = noise(i,j);          			
          			}
        		}
  			}
  		}
