...
void forcing(mat & famp, double kf, double dk){
...
for(int j =0; j < Ny/2; j++){
	for(int i =0; i < Nx/2; i++){
		...
		if( abs(k-kf) < dk){
			...
			energy_inj += pow(famp(i,j),2.0)*(k*k);
			forcing_modes += 1;
		}
		...
		if( abs(k-kf) < dk){
			...
			energy_inj += pow(famp(Nx-i-1,j),2.0)*(k*k);
			forcing_modes += 1;
		}
		...
		if( abs(k-kf) < dk){
			...
			energy_inj += pow(famp(i,Ny-j-1),2.0)*(k*k);
			forcing_modes += 1;
		}
		...
		if( abs(k-kf) < dk){
			famp(Nx-i-1,Ny-j-1) = Amp;
			waveaction_inj += pow(famp(Nx-i-1,Ny-j-1),2.0);
			energy_inj += pow(famp(Nx-i-1,Ny-j-1),2.0)*(k*k);
			forcing_modes += 1;
		}
	}
}
...
void forcing2(mat & famp, double kf, double dk){
